...
def ensure_directories():
    ...
    try:
        ...
        warning(f"[警告] 无法加载配置文件 {config_path}: {e}, 使用默认目录设置")
    ...
    for folder in required_dirs:
        ...
        if os.path.exists(folder_path):
            ...
            if os.path.isdir(folder_path):
                debug(f"目录 '{folder_path}' 已存在。")
            else:
                info(f"[警告] 路径 '{folder_path}' 存在但不是目录，将重新创建。")
                try:
                    import shutil
                    shutil.rmtree(folder_path)  # 删除文件或目录树
                    os.makedirs(folder_path)
                    debug(f"目录 '{folder_path}' 已重新创建。")
                except Exception as e:
                    error(f"[错误] 重新创建目录 '{folder_path}' 失败: {e}")
                    return False
        else:
            # 目录不存在，创建目录
            try:
                os.makedirs(folder_path)
                debug(f"目录 '{folder_path}' 已创建。")
            except Exception as e:
                error(f"[错误] 创建目录 '{folder_path}' 失败: {e}")
                return False
...
def main():
    ...
    debug(f"将启动的Flask应用模块: {APP_FLASK_MODULE}")
