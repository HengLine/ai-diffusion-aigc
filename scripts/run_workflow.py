...
class ComfyUIRunner:
        ...
        try:
            ...
            if "nodes" in workflow:
                ...
                for node in workflow["nodes"]:
                    ...
                    if "type" in node and "class_type" not in node:
                        node["class_type"] = node["type"]
                        print(f"为节点 {node.get('id', 'unknown')} 添加了class_type属性")
            ...
            # 发送POST请求运行工作流
            print(f"正在向 {self.api_url}/prompt 发送请求...")
            ...
            if not isinstance(response_json, dict):
                print(f"API响应不是字典类型，而是: {type(response_json)}")
                return False
            
            prompt_id = response_json.get("prompt_id")
            if not prompt_id:
                print(f"无法获取prompt_id，响应内容: {response_json}")
                return False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not isinstance(history, dict):
                        print(f"历史记录不是字典类型，而是: {type(history)}")
                        time.sleep(1)
                        continue
                    ...
                    if prompt_id in history:
                        ...
                        if not isinstance(prompt_data, dict):
                            print(f"prompt_data不是字典类型，而是: {type(prompt_data)}")
                            time.sleep(1)
                            continue
    ...
    def _get_workflow_outputs(self, prompt_id: str, output_path: str) -> bool:
        ...
        try:
            ...
            print(f"[ComfyUI API] 获取工作流历史记录: {api_endpoint}")
            response = requests.get(api_endpoint)
            
            if response.status_code != 200:
                print(f"[ComfyUI API] 获取历史记录失败: 状态码={response.status_code}, 响应内容={response.text}")
                return False
            
            print(f"[ComfyUI API] 获取历史记录成功，状态码: {response.status_code}")
            history = response.json()
            
            if not isinstance(history, dict) or prompt_id not in history:
                print(f"[ComfyUI API] 历史记录格式不正确或找不到指定的prompt_id: {prompt_id}")
                return False
            
            prompt_data = history[prompt_id]
            if not isinstance(prompt_data, dict) or "outputs" not in prompt_data:
                print(f"[ComfyUI API] 找不到工作流输出，prompt_data格式: {type(prompt_data)}")
                return False
            
            outputs = prompt_data["outputs"]
            if not isinstance(outputs, dict):
                print(f"[ComfyUI API] outputs不是字典类型，而是: {type(outputs)}")
                return False
            
            print(f"[ComfyUI API] 找到 {len(outputs)} 个输出节点")
            # 查找图像或视频输出
            for node_id, node_output in outputs.items():
                print(f"[ComfyUI API] 检查输出节点: {node_id}")
                # 确保node_output是字典类型
                if not isinstance(node_output, dict):
                    print(f"[ComfyUI API] node_output不是字典类型，node_id: {node_id}, 类型: {type(node_output)}")
                    continue
                
                # 处理图像输出
                if "images" in node_output:
                    print(f"[ComfyUI API] 找到图像输出，节点ID: {node_id}")
                    # 确保images是列表类型
                    if not isinstance(node_output["images"], list):
                        print(f"[ComfyUI API] images不是列表类型，node_id: {node_id}")
                        continue
                    
                    print(f"[ComfyUI API] 图像数量: {len(node_output['images'])}")
                    for image_info in node_output["images"]:
                        # 确保image_info是字典类型
                        if not isinstance(image_info, dict):
                            print(f"[ComfyUI API] image_info不是字典类型")
                            continue
                        
                        # 检查必要的键是否存在
                        if not all(key in image_info for key in ['filename', 'subfolder', 'type']):
                            print(f"[ComfyUI API] image_info缺少必要的键")
                            continue
                        
                        print(f"[ComfyUI API] 图像信息: 文件名={image_info['filename']}, 子文件夹={image_info['subfolder']}")
                        # 获取图像数据
                        try:
                            view_url = f"{self.api_url}/view?filename={image_info['filename']}&subfolder={image_info['subfolder']}&type={image_info['type']}"
                            print(f"[ComfyUI API] 获取图像数据: {view_url}")
                            image_data = requests.get(view_url)
                            if image_data.status_code == 200:
                                # 保存图像到指定路径
                                print(f"[ComfyUI API] 图像数据获取成功，保存到: {output_path}")
                                with open(output_path, 'wb') as f:
                                    f.write(image_data.content)
                                print(f"[ComfyUI API] 图像保存成功")
                                return True
                            else:
                                print(f"[ComfyUI API] 图像数据获取失败，状态码: {image_data.status_code}")
                        except Exception as img_err:
                            print(f"[ComfyUI API] 获取或保存图像时出错: {str(img_err)}")
                            continue
                
                # 处理视频输出
                elif "videos" in node_output:
                    print(f"[ComfyUI API] 找到视频输出，节点ID: {node_id}")
                    # 确保videos是列表类型
                    if not isinstance(node_output["videos"], list):
                        print(f"[ComfyUI API] videos不是列表类型，node_id: {node_id}")
                        continue
                    
                    print(f"[ComfyUI API] 视频数量: {len(node_output['videos'])}")
                    for video_info in node_output["videos"]:
                        # 确保video_info是字典类型
                        if not isinstance(video_info, dict):
                            print(f"[ComfyUI API] video_info不是字典类型")
                            continue
                        
                        # 检查必要的键是否存在
                        if not all(key in video_info for key in ['filename', 'subfolder', 'type']):
                            print(f"[ComfyUI API] video_info缺少必要的键")
                            continue
                        
                        print(f"[ComfyUI API] 视频信息: 文件名={video_info['filename']}, 子文件夹={video_info['subfolder']}")
                        # 获取视频数据
                        try:
                            view_url = f"{self.api_url}/view?filename={video_info['filename']}&subfolder={video_info['subfolder']}&type={video_info['type']}"
                            print(f"[ComfyUI API] 获取视频数据: {view_url}")
                            video_data = requests.get(view_url)
                            if video_data.status_code == 200:
                                # 保存视频到指定路径
                                print(f"[ComfyUI API] 视频数据获取成功，保存到: {output_path}")
                                with open(output_path, 'wb') as f:
                                    f.write(video_data.content)
                                print(f"[ComfyUI API] 视频保存成功")
                                return True
                            else:
                                print(f"[ComfyUI API] 视频数据获取失败，状态码: {video_data.status_code}")
                        except Exception as vid_err:
                            print(f"[ComfyUI API] 获取或保存视频时出错: {str(vid_err)}")
                            continue
            
            print("[ComfyUI API] 工作流没有产生图像或视频输出")
            return False
        except Exception as e:
            print(f"[ComfyUI API] 获取工作流输出时出错: {str(e)}")
            return False
