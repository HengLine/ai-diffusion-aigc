...
@text_to_video_bp.route('/api/text_to_video', methods=['POST'])
def api_text_to_video():
    ...
    debug(f"[{request_id}] 接收到文生视频API请求")
    ...
    try:
        ...
        # 记录任务信息
        debug(f"[{request_id}] 开始处理文生视频任务 - prompt: {prompt[:50]}...")
        ...
        error(f"[{request_id}] API请求处理异常: {str(e)}")
        return jsonify({
            'success': False,
