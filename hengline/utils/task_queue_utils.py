...
class TaskQueueManager:
    ...
    def _execute_callback_with_timeout(self, task: Task, timeout: int = 120):
        """执行任务回调函数，并设置超时时间"""
        result = None
        exception = None
    ...
    def get_queue_status(self, task_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        # 计算历史任务总数（这里简单返回0，实际应用中可能需要从数据库或文件中获取）
        total_history_tasks = 0
    ...
    def _load_task_history(self):
        ...
        try:
            ...
            for file_path in history_files:
                if loaded_days >= max_days:
                    break
                
                file_name = os.path.basename(file_path)
                
                try:
                    # 优先处理今天和昨天的文件，这些可能包含运行中的任务
                    if today in file_name or yesterday in file_name:
                        with open(file_path, 'r', encoding='utf-8') as f:
                            tasks_data = json.load(f)
                        
                        # 只加载状态为queued或running的任务，以及最近完成的任务
                        for task_data in tasks_data:
                            # 检查任务状态是否需要加载
                            load_task = False
                            if task_data.get('status') in ['queued', 'running']:
                                load_task = True
                            elif 'end_time' in task_data and task_data['end_time'] > time.time() - 3600:  # 1小时内完成的任务
                                load_task = True
                        ...
                        # 对于其他日期的文件，只做基本验证，不加载所有任务
                        # 快速检查文件是否有效，不加载完整内容
                        with open(file_path, 'r', encoding='utf-8') as f:
                            # 只读取文件头和文件尾，确保文件格式正确
                            f.read(100)
                            f.seek(max(0, os.path.getsize(file_path) - 100), 0)
                            f.read(100)
            ...
            debug(f"已加载任务历史，共 {loaded_task_count} 个活跃任务")
        except Exception as e:
