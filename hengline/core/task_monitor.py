...
class TaskMonitor:
    ...
    def _check_tasks(self, current_thread: threading.current_thread()):
        ...
        if not self._task_check_lock.acquire(blocking=False):
            debug(
                f"跳过任务检查 - 已有线程在执行检查，实例ID: {self.instance_id}, 进程ID: {self.process_id}, 线程ID: {current_thread.ident}, 线程名称: {current_thread.name}")
            return
        ...
        try:
            ...
            eligible_tasks = [task for task in today_tasks if
                              task.get('execution_count', 0) <= self.max_execution_count]
    ...
    def _query_comfyui_api_with_timeout(self, prompt_id: str, timeout: int = 10) -> Optional[
