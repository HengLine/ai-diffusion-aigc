...
print(f'初始任务状态: status={task.status}, 队列大小={task_manager.task_queue.qsize()}')
...
# 验证结果
print(f'检查后任务状态: status={task.status}, 队列大小={task_manager.task_queue.qsize()}')

# 验证修复是否有效
if task_manager.task_queue.qsize() > 0:
    print('测试成功: 排队中的任务已被正确地加入队列!')
else:
    print('测试失败: 排队中的任务没有被加入队列!')